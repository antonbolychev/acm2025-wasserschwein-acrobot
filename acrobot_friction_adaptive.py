...
output_dir = root_dir / "gfx" / "adaptive_control"
...
class Acrobot:
    ...
    def __init__(
        ...
        self.gamma =  0.00512#0.0051#0.00484  # tuning parameter for adaptive control
        self.b2_hat = [0.0]  # initial estimate of b2
        self.tau2_list = [] # list to store control input

        self.t_prev = 0.0  # previous time step for adaptive control
    ...
    def controller(self, t, state):
        ...
        if self.is_energy_based_only or (
            ...
            if not self.is_switched:
                print("Switching to PD controller")
                Acrobot.switch_time = t
    ...
    def simulate(self, t_span, initial_state, t_eval=None):
        ...
        self.tau2_list = np.array(self.tau2_list)
        self.tau2_list = self.tau2_list[np.linspace(0, len(self.tau2_list)-1, len(sol.t), dtype=int)]

        return sol.t, sol.y, self.tau2_list
...
def plot_results(t, y, tau2, t_span, output_dir):
    ...
    plt.xlabel("q2 (rad)")
    plt.ylabel("dq2 (rad/s)")
    plt.title("Phase Portrait of Link 2")
    plt.grid(True)
    plt.legend()
    ...
    plt.axvline(Acrobot.switch_time, color="r", linestyle="--", label="Switch Time")
    plt.xlim(t_span)
    plt.xlabel("Time (s)")
...
def animate_acrobot(t, y, t_span):
    ...
    b_hat_text = ax.text(0.02, 0.75, "", transform=ax.transAxes)
    ...
    def animate(i):
        ...
        # Show b2_hat
        b_hat_text.set_text(f"b2_hat = {acrobot.b2_hat[-1]:.2f}")
...
def plot_friction_coeff(t, b2_list, b2_true,output_dir):
    """Plot the estimated friction coefficient over time"""
    plt.figure(figsize=(10, 6))
    b2_list = np.array(b2_list)
    plt.plot(t, b2_list[np.linspace(0, len(b2_list)-1, len(t), dtype=int)], label="Estimated b2")
    plt.axhline(b2_true, color="r", linestyle="--", label="True b2")
    plt.xlabel("Time (s)")
    plt.ylabel("Estimated Friction Coefficient")
    plt.title("Estimated Friction Coefficient Over Time")
    plt.grid()
    plt.savefig(output_dir / "friction_coeff.png")
    print(f"Friction coefficient plot saved to {output_dir}/friction_coeff.png")
...
if __name__ == "__main__":
    ...
    plot_friction_coeff(t, acrobot.b2_hat, acrobot.b2, params.output_dir)
