...
class Acrobot:
    ...
    def controller(self, t, state):
        ...
        if self.is_energy_based_only or (
            ...
            target_control = self.energy_based_controller(t, state)
        else:
            self.is_switched = True
            target_control = self.pd_controller(t, state)
        u = tau2 - self.gamma * (tau2 - target_control)
    ...
    def dynamics(self, t, state):
        ...
        # Acrobot has no actuation at joint 1
        tau = np.array([0, tau2])
        ...
        return [dq1, dq2, ddq[0], ddq[1], dtau2]
...
if __name__ == "__main__":
    ...
    initial_state = [-1.4, 0.0, 0.0, 0.0, 0.0]  # [q1, q2, dq1, dq2]
